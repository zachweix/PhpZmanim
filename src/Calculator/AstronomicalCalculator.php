...
abstract class AstronomicalCalculator
{
	...
	*/
	...
	public function setRefraction(float $refraction): self
	{
	...
	public function getSolarRadius(): float
	{
		return $this->solarRadius;
	}

	public function setSolarRadius(float $solarRadius): self
	{
		if ($solarRadius < 0 || is_nan($solarRadius)) {
			throw new InvalidArgumentException('Solar radius must be a non-negative number');
		}
	...
	public function getEarthRadius(): float
	{
		return $this->earthRadius;
	}

	public function setEarthRadius(float $earthRadius): self
	{
	...
	*/
	...
	abstract public function getUTCSunrise(Carbon $date, GeoLocation $geo, float $zenith, bool $adjustForElevation): float;

	abstract public function getUTCSunset(Carbon $date, GeoLocation $geo, float $zenith, bool $adjustForElevation): float;

	abstract public function getUTCNoon(Carbon $date, GeoLocation $geo): float;

	abstract public function getUTCMidnight(Carbon $date, GeoLocation $geo): float;

	abstract public function getTimeAtAzimuth(Carbon $date, GeoLocation $geo, float $azimuth): float;

	abstract public function getSolarElevation(Carbon $datetime, GeoLocation $geo): float;

	abstract public function getSolarAzimuth(Carbon $datetime, GeoLocation $geo): float;
	...
	*/
	...
	private function getElevationAdjustment(float $elevation): float
	{
		return rad2deg( acos($this->earthRadius / ($this->earthRadius + ($elevation / 1000.0))));
	}

	private function getApparentSolarRadius(?Carbon $date): float
	{
		return is_null($date) ? ((float) 16 / 60.0) : self::SOLAR_RADIUS_BY_DAY_OF_YEAR[ $date->copy()->year(2050)->dayOfYear - 1 ];
	}

	protected function adjustZenith(float $zenith, float $elevation, ?Carbon $date = null): float
	{
	...
	*/
	...
	public function equals(AstronomicalCalculator $calculator): bool
	{
	...
	/*
	|--------------------------------------------------------------------------
	| SOLAR RADIUS CONSTANTS
	|--------------------------------------------------------------------------
	*/

	const SOLAR_RADIUS_BY_DAY_OF_YEAR = [
