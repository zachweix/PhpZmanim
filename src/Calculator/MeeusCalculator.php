...
class MeeusCalculator extends AstronomicalCalculator
{
	...
	*/
	...
	public function getTimeAtAzimuth(Carbon $date, GeoLocation $geo, float $azimuth): float
	{
		...
		for ($i = 0; $i < 3; $i++) {
			$julianCenturies = $this->getJulianCenturiesFromJulianDay($julianDay + $dateTime);
	...
	*/
	...
	private static function getDeltaT(float $julianDay): float
	{
	...
	{
		$sun = self::getSunApparentEclipticCoordinates($julianCenturies);
		$lambda = $sun[0];
		$beta = $sun[1];
		$epsilon = self::getTrueObliquity($julianCenturies);
		$sinDec = sin(deg2rad($beta)) * cos(deg2rad($epsilon)) + cos(deg2rad($beta)) * sin(deg2rad($epsilon)) * sin(deg2rad($lambda));
		return rad2deg(asin($sinDec));
	}

	private static function getSunRightAscension(float $julianCenturies): float
	{
		$sun = self::getSunApparentEclipticCoordinates($julianCenturies);
		$lambda = $sun[0];
		$beta = $sun[1];
		$epsilon = self::getTrueObliquity($julianCenturies);
		$y = sin(deg2rad($lambda)) * cos(deg2rad($epsilon)) - tan(deg2rad($beta)) * sin(deg2rad($epsilon));
		$x = cos(deg2rad($lambda));
		$ra = rad2deg(atan2($y, $x));
		return fmod(fmod($ra, 360) + 360, 360);
	}

	private static function getSunApparentEclipticCoordinates(float $julianCenturies): array
	{
		$tau = $julianCenturies / 10.0;
		$earthL = rad2deg(self::sumSeries(self::EARTH_L, $tau));
		$earthB = rad2deg(self::sumSeries(self::EARTH_B, $tau));
		$earthR = self::sumSeries(self::EARTH_R, $tau);

		$theta = $earthL + 180.0;
		$beta = -$earthB;

		$lambdaPrime = $theta - 1.397 * $julianCenturies - 0.00031 * $julianCenturies * $julianCenturies;
		$deltaLongFK5 = -0.09033 + 0.03916 * (cos(deg2rad($lambdaPrime)) + sin(deg2rad($lambdaPrime))) * tan(deg2rad($beta));
		$deltaLatFK5 = 0.03916 * (cos(deg2rad($lambdaPrime)) - sin(deg2rad($lambdaPrime)));
		$theta += $deltaLongFK5 / 3600.0;
		$beta += $deltaLatFK5 / 3600.0;

		$nut = self::getNutation($julianCenturies);
		$aberration = -20.4898 / $earthR;
		$lambda = $theta + $nut[0] / 3600.0 + $aberration / 3600.0;

		return [fmod(fmod($lambda, 360) + 360, 360), $beta];
	}

	private static function getTrueObliquity(float $julianCenturies): float
	{
		...
		$deltaEpsilon = self::getNutation($julianCenturies)[1] / 3600.0;
		return $epsilon0 + $deltaEpsilon;
	}

	private static function getNutation(float $julianCenturies): array
	{
		$t = $julianCenturies;
		$omega = 125.04452 - 1934.136261 * $t + 0.0020708 * $t * $t + $t * $t * $t / 450000.0;
		$lSun = 280.4665 + 36000.7698 * $t;
		$lMoon = 218.3165 + 481267.8813 * $t;
		$deltaPsi = -17.20 * sin(deg2rad($omega)) - 1.32 * sin(deg2rad(2 * $lSun))
				- 0.23 * sin(deg2rad(2 * $lMoon)) + 0.21 * sin(deg2rad(2 * $omega));
		$deltaEpsilon = 9.20 * cos(deg2rad($omega)) + 0.57 * cos(deg2rad(2 * $lSun))
	...
	private static function getEquationOfTime(float $julianCenturies): float
	{
		...
		$alpha = self::getSunRightAscension($julianCenturies);
		$nut = self::getNutation($julianCenturies);
		$epsilon = self::getTrueObliquity($julianCenturies);
		$e = $l0 - 0.0057183 - $alpha + ($nut[0] / 3600.0) * cos(deg2rad($epsilon));
	...
	{
		$ratio = cos(deg2rad($zenith)) / (cos(deg2rad($latitude)) * cos(deg2rad($solarDeclination)))
				- tan(deg2rad($latitude)) * tan(deg2rad($solarDeclination));
	...
	*/
	...
	private function getJulianCenturiesFromJulianDay(float $julianDayUT): float
	{
		$jde = $julianDayUT;
		if ($this->applyDeltaT) {
			$jde += self::getDeltaT($julianDayUT) / 86400.0;
		}
		return ($jde - self::JULIAN_DAY_JAN_1_2000) / self::JULIAN_DAYS_PER_CENTURY;
	}

	private function getSolarNoonMidnightUTC(float $julianDay, float $longitude, $solarEvent): float
	{
		$base = ($solarEvent == self::NOON) ? 720 : 1440;
		$tnoon = $this->getJulianCenturiesFromJulianDay($julianDay + $longitude / 360.0);
		$equationOfTime = self::getEquationOfTime($tnoon);
		$solNoonUTC = $base + ($longitude * 4) - $equationOfTime;

		for ($i = 0; $i < 3; $i++) {
			$newt = $this->getJulianCenturiesFromJulianDay($julianDay + $solNoonUTC / 1440.0);
	...
	private function getSunRiseSetUTC(Carbon $date, float $latitude, float $longitude, float $zenith, $solarEvent): float
	{
		...
		$tnoon = $this->getJulianCenturiesFromJulianDay($julianDay + $noonmin / 1440.0);
		$equationOfTime = self::getEquationOfTime($tnoon);
		...
		for ($i = 0; $i < 2; $i++) {
			$newt = $this->getJulianCenturiesFromJulianDay($julianDay + $timeUTC / 1440.0);
			$equationOfTime = self::getEquationOfTime($newt);
	...
	private function getSolarElevationAzimuth(Carbon $datetime, GeoLocation $geo, bool $isAzimuth): float
	{
		...
		$jc = $this->getJulianCenturiesFromJulianDay(self::getJulianDay($utc) + $fractionalDay);
		...
		$elevation = (90.0 - $zenithDeg) + $this->adjustElevationForRefraction(90.0 - $zenithDeg);
		...
		return $isAzimuth ? fmod($azimuth + 360, 360) : $elevation;
	}
